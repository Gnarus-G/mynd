...
fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        Some(c) => match c {
            ...
            Command::Url => println!("{}", todo::config::web_url()?),
...
mod manageconfigcli {
    ...
    use todo::config::{self, store_config};

    #[derive(Args, Debug)]
    pub struct ConfigProps {
        #[arg(short = 'f', long = "format")]
        /// The storage format of the collection of todo items.
        storage_format: Option<config::SaveFileFormat>,

        #[arg(long)]
        /// Canonical HTTPS URL used by the PWA launcher.
    ...
    impl ConfigArgs {
        pub fn handle(self) -> anyhow::Result<()> {
            match self.command {
                ConfigActions::Set(ConfigProps {
                    storage_format,
                    web_url,
                }) => {
                    if storage_format.is_none() && web_url.is_none() {
                        anyhow::bail!("provide --format or --web-url");
                    }
                    if let Some(url) = web_url.as_deref() {
                        config::validate_web_url(url)?;
                    }
